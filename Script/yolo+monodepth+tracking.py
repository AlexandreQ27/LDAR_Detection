...
def detect(image):
    ...
    if pred[0].shape[0] != 0:
        # 保存检测帧的图片数组信息用于产生深度图
        np.save('C:/Users/14471/Desktop/LDAR/yolov5/cache2/frame_{}.npy'.format(frame_cnt), input_image)
        os.system(
              r"C:/Users/14471/anaconda3/envs/monodepth2/python.exe C:/Users/14471/Desktop/LDAR/monodepth2/mono_test_npy.py --image_path C:/Users/14471/Desktop/LDAR/yolov5/cache2/frame_{}.npy --model_name mono+stereo_no_pt_640x192".format(frame_cnt))
        depth_array = np.load('C:/Users/14471/Desktop/LDAR/yolov5/cache2/frame_{}_disp.npy'.format(frame_cnt))
        # 处理检测结果
                        ...
                        if class_names[itor] == class_names[int(cls)]:
                            ...
                            if sum(object_frames[class_names[int(cls)]]) >= 2 * min_frames:
                                ...
                                if distance < 0.2:
                                    ...
                                    print(center)  # 输出：(175, 225)
                                    centers.append(center)
                                    for i in range(1, len(centers)):
                                        # 计算当前时间步骤的中心点和上一个时间步骤的中心点
                                        center1 = centers[i - 1]
                                        center2 = centers[i]
                                    ...
                                    # 清空画布
                                    cv2.rectangle(canvas, (0, 0), (800, 480), (0, 0, 0), -1)
                            else:
                                # 清空画布
                                cv2.rectangle(canvas, (0, 0), (800, 480), (0, 0, 0), -1)
                        else:
    ...
    return canvas,image, depth_mean


# 加载视频并进行检测
cap = cv2.VideoCapture(str('C:/Users/14471/Desktop/LDAR/N01155150.mp4'))
...
while cap.isOpened():
    ...
    canvas, result, distance = detect(frame)
    frame_cnt += 1
